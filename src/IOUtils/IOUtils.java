...
public class IOUtils {
    ...
    public String getUserPassword(String prompt) {
        ...
        do {
            ...
            // Check to see if input meets password criteria
            if (userInput.length() < 8 || userInput.length() > 20) {
                System.out.println("Password must be between 8 and 20 characters.");
            } else if (!userInput.matches("^(?=.*[0-9])(?=.*[a-zA-Z])(?=\\S+$).*$")) {
                // Password should contain at least one digit, one letter, and no spaces
                System.out.println("Invalid password format. It should contain at least one digit, one letter, and no spaces.");
            } else {
                valid = true;
            }
